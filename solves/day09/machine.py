...
class IntcodeMachine:
    """Default Intcode machine with manual input-output interface."""
    ...
    async def interp(self):
        ...
        # Fire and move on
        await method(*args, modes=param_modes)

    def read_value(self, param: int, mode: Mode):
    ...
    def write_value(self, param: int, mode: Mode, value: int):
        if mode == 0:  # absolute mode
    ...
    async def execute_01(self, fst_param, snd_param, res_param, *, modes, **_):
        ...
        """
        fst_value = self.read_value(fst_param, modes[0])
        snd_value = self.read_value(snd_param, modes[1])
        result = fst_value + snd_value
        self.write_value(res_param, modes[2], result)
        self.pc += 4
    ...
    async def execute_03(self, target_param, *, modes, **_):
        """
        Opcode 3: Save input integer to target
        """
        value = await self.get_input()
        self.write_value(target_param, modes[0], value)
        self.pc += 2

    async def execute_04(self, source_param, *, modes, **_):
        """
        Opcode 4: Output integer from source location
        """
        value = self.read_value(source_param, modes[0])
        await self.set_output(value)
        self.pc += 2
    ...
    async def execute_05(self, cond_param, pos_param, *, modes, **_):
        ...
        """
        cond_value = self.read_value(cond_param, modes[0])
        pos_value = self.read_value(pos_param, modes[1])
    ...
    async def execute_09(self, adjust_param, *, modes, **_):
        ...
        """
        adjust_value = self.read_value(adjust_param, modes[0])
...
class PipeIntcodeMachine(IntcodeMachine):
    """Intcode machine with input-output async queues."""

    input_values: asyncio.Queue
    output_values: asyncio.Queue

    def __init__(self, program: Program):
        super().__init__(program)
        self.input_values = asyncio.Queue()
        self.output_values = asyncio.Queue()

    async def get_input(self) -> int:
        value = await self.input_values.get()
        return value

    async def set_output(self, value: int) -> None:
        await self.output_values.put(value)
